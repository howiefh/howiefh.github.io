...
**char、byte、short类型的数值进行移位时，移位进行之前会先转为int，最后结果也是int型。只有右操作数的低5位才是有用的。这样可以防止我们移位超过int型值所具有的位数**。没有任何移位操作符可以让一个数丢弃所有的位，int是32位。移位操作符只有右操作数低5位有效，实际是对右操作数对32取模后移位，`50<<33`相当于`50<<(33%32)`
